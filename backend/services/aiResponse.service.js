...
export const retrieveAIResponse = async (tripId, type = 'itinerary', responseId = null) => {
  try {
    ...
    if (responseId) {
      ...
      aiResponse = await AIResponse.findOne({ tripId, type }).sort({ createdAt: -1 });
    }
...
export const listAIResponses = async (tripId, type = null) => {
  try {
    ...
    const aiResponses = await AIResponse.find(query).sort({ createdAt: -1 });
...
export const cleanupOldAIResponses = async (tripId, type = 'itinerary', keepCount = 3) => {
  try {
    ...
    const allVersions = await AIResponse.find({ tripId, type })
      .sort({ createdAt: -1 });
    ...
    if (allVersions.length > keepCount) {
      ...
      const result = await AIResponse.deleteMany({ _id: { $in: deleteIds } });
